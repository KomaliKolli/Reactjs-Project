...
const Recipe = () => {
    ...
    // Extract id logic inside useEffect when item changes
    useEffect(() => {
        if (item) {
            const strYoutube = item.strYoutube;
            const str = strYoutube.split("=");
            setId(str[str.length - 1]);
        }
    }, [item]);
        ...
        <>
            {
                (!item) ? "" : <div className="content">
                    ...
                    <div className="video">
                        <iframe width="100%" height="515" title="recipeVideo" src={`https://www.youtube.com/embed/${id}`}></iframe>
                    </div>
                </div>
            }
        </>
